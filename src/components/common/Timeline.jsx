export default function Timeline({ items }) {
  return (
  <ol className="relative ml-1 space-y-5 border-l border-base-700 sm:ml-4 lg:ml-10">
      {items.map((item) => {
        const summaryClass = ["text-sm text-base-200", item.summaryPreserveWhitespace ? "whitespace-pre-line" : ""]
          .filter(Boolean)
          .join(" ");

        return (
          <li key={item.title} className="group relative pl-10 sm:pl-12">
            ...
            <div className="space-y-3 rounded-2xl border border-base-800 bg-base-800/70 p-5 transition group-hover:border-support-teal/40 group-hover:bg-base-800/95">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <h3 className="text-xl font-semibold text-base-100">{item.title}</h3>
                {item.timeframe ? (
                  <span className="text-xs uppercase tracking-[0.3em] text-support-teal/80">{item.timeframe}</span>
                ) : null}
              </div>
              ...
              {Array.isArray(item.bullets) && item.bullets.length > 0 ? (
                <ul className="space-y-2 text-sm text-base-200">
                  {item.bullets.map((bullet, idx) => (
                    <li key={`${item.title}-bullet-${idx}`} className="flex gap-2">
